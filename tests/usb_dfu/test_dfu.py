...
def test_dfu(pytestconfig, board, config, dfuapp):
    ...
    clear_upgrade = Path("../clear_upgrade/bin/clear_upgrade.xe")
    if clear_upgrade.exists():
        ret = subprocess.run(f"xrun --xscope --adapter-id {adapter_dut} {clear_upgrade}".split(), capture_output=True, text=True)
        print(f"Clear upgrade check:\n{ret.stdout}")
    ...
    with AppUsbAudDut(adapter_dut, board, config, xflash=True, writeall=writeall) as dut:
        ...
        if initial_version != "1.23":
            print(f"Unexpected initial version {initial_version}, attempting to revert to factory image before starting test, expected 1.23")
            dfu_test.revert_factory()
            initial_version = dfu_test.get_bcd_version()

        assert initial_version == "1.23", f"Initial version {initial_version} didn't match expected 1.23"
